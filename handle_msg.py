...
def handle_msg2reply(userMes):
    ...
    if mesType == 1: #予定表示
        ...
        prev_userMes = sql.ref_log(2)[COL_MES_TXT]
        [prev_mesType, prev_schedule] = new_trans.trans(prev_userMes) 
        if prev_mesType == 4:
            reply_mes = ""
            if type(prev_schedule[COL_DATETIME]) is datetime.date:
                sql.delete(prev_schedule[COL_DATETIME].strftime('%Y-%m-%d'))
                reply_mes = str(prev_schedule[COL_DATETIME].month) + "月" + str(prev_schedule[COL_DATETIME].day) +"日の"
            else:
                sql.delete0(0)
            reply_mes += "予定を削除しました！"
        else:
